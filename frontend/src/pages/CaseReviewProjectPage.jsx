...
import BatchImportModal from '../components/caseReview/BatchImport'
...
export default function CaseReviewProjectPage() {
  ...
  // 批量导入在电脑上放在病历工具栏，手机上收进「···」
  const menuActions = project ? [
    ...(isMobile && canUpload ? [{ label: '批量导入病历', onClick: () => { switchTab('cases'); setBatchOpen(true) } }] : []),
  ...
  return <div className="memo-page cr-project">
    ...
    {!project.is_member && <div className="cr-readonly-banner">这是演示项目，你不是成员，只能查看。</div>}
    ...
    {tab === 'cases' && <div className="cr-cases">
      <div className="cr-case-toolbar">
        ...
        {canUpload && <button type="button" className="btn-secondary cr-desktop-only cr-batch-button" onClick={() => setBatchOpen(true)}>批量导入</button>}
        {canUpload && <button type="button" className="btn-primary cr-desktop-only" onClick={() => setCaseForm(true)}>新建病历</button>}
      </div>
      {!cases.length && <div className="empty-state cr-empty">还没有病历。{canUpload ? (isMobile ? '点右上角「＋」逐份添加，或在「···」里批量导入。' : '点「新建病历」逐份添加，或用「批量导入」一次导入整个文件夹。') : ''}</div>}
    ...
    {caseForm && <CaseFormModal projectId={project.id} members={reviewers} defaultCode={nextCaseCode(cases.map(item => item.code))} onClose={() => setCaseForm(false)} onSaved={id => navigate(`/case-review/${project.id}/cases/${id}`)} />}
