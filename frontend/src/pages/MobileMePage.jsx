...
const ICONS = {
  ...
  caseReview: <><path d="M13.5 3.5H7a1.5 1.5 0 0 0-1.5 1.5v14A1.5 1.5 0 0 0 7 20.5h10a1.5 1.5 0 0 0 1.5-1.5V8.5z" /><path d="M13.5 3.5v5h5" /><path d="M9 14.2l2 2 4-4.2" /></>,
}
...
export default function MobileMePage() {
  ...
  useEffect(() => {
    if (isMobile) getCaseReviewSummary().then(summary => setCaseReviewCount(summary.waiting_count)).catch(() => {})
  }, [isMobile])
  ...
  // 每项：[地址, 名称, 图标, 图标底色, 角标数]
  const groups = [
    [['/case-review', '病历审阅', 'caseReview', '#ff2d55', caseReviewCount]],
  ...
  return <div className="m-me">
    ...
    {groups.map((items, index) => <section className="m-list" key={index}>{items.map(([to, label, icon, color, badge]) =>
      <Link key={to} to={to} className="m-cell m-cell-icon">
        ...
        {badge > 0 && <span className="m-cell-badge">{badge > 99 ? '99+' : badge}</span>}
